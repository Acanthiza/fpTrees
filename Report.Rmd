...
subtitle: "Analysis of Katarapko Tree Condition Data"
author:
...
# Other constants
  ...
  useName <- "Katarapko tree condition"
  
  #wordDir <- "https://subversion.dev.env.sa.gov.au/svn/repos/science/fpTrees"
...
All data analysis and report writing was done in a single scripted workflow (script file: [`r list.files(here::here(),pattern="Report.Rmd",recursive=TRUE)`](`r paste0(here::here(),"/",list.files(here::here(),pattern="Report.Rmd",recursive=TRUE))`) using the programs 'R' and 'R-studio'.
...
All data, code and outputs are stored in [`r here::here()`](`r here::here()`).
...
There were `r nrow(datExp)` trees available for analysis (see table below). The distribution of these trees on the Katarapko floodplain is shown in Figure \@ref(fig:treesMap).
...
```{r modFit, fig.height = 8, fig.cap = "Model fit"}
  ...
  # Model fit
    a <- pp_check(mod) + coord_cartesian(xlim = c(0,quantile(mod$fitted.values,probs=0.05))) + labs(title = "0-quantile(0.05)")
